...
def read_data_from_csv():
    try:
        ...
        data_file_path = os.path.join(package_dir, "sct_input.csv")
        aws_file_path = os.path.join(package_dir, "awssctcomplexitymatrix.csv")
...
def execution(data,aws_df,args,conn):
    ...
    try:
        ...
        if len(merged_df.index)!=0:
            ...
            stacked_bar_chart_1(args, merged_df)
            stacked_bar_chart_2(args, merged_df) 
...
def render_html(args,template,results,titles,descriptions,exc_summary):
    try:
...
def stacked_bar_chart_1(args, merged_df): 
...
def stacked_bar_chart_2(args, merged_df):
        complexities = {}
        count_labels = []
...
def main():
    ...
    if validate_extension(conn):
        ...
        render_html(args,template,results,titles,descriptions,exc_summary)
